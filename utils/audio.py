...
def stft_(x, fs=config.fs, fftsize=config.fftsize, overlap=config.overlap, window='hann', padded=True):
...
def istft(X,  fs=config.fs, fftsize=config.fftsize, overlap=config.overlap, window='hann'):
...
def create_spectrogram(x, sr, fs=config.fs, fftsize=config.fftsize, overlap=config.overlap, window=config.window, padded=True):
