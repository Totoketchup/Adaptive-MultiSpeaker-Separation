...
def main(args):
	...
	####
	model = DPCL.load(args.model_folder, d)
	...
	for epoch in range(nb_epochs):
		for b in range(nb_batches_train):
			...
			t = time.time()
			c = model.train(X_mix, X_non_mix, step)
			t_f = time.time()
			...
			if step%args.validation_step == 0:
				...
				for _ in range(nb_batches_valid):
					...
					cost = model.valid_batch(X_v_mix, X_v_non_mix)
					costs.append(cost)
	...
	for _ in range(nb_batches_test):
		...
		cost = model.valid_batch(X_t_mix, X_t_non_mix)
		costs.append(cost)
